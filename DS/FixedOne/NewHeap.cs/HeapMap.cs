...
class Program {
    ...
    public static void Test1 () {
        ...
        for (int i = 0; i < 123; i++) {
            ...
            if (heap.Remove (i + 1)) Console.WriteLine ("sfds");
...
[DebuggerDisplay ("HeapCount = {Count}")]
public abstract class HeapMap<TKey, TValue> : IDictionary<TKey, TValue>
    ...
    where TValue : IComparable<TValue> {
        ...
        #region implement generics function
        ...
        bool Delete (TKey key, TValue value = default (TValue), bool checkValue = false) {
            ...
            for (int i = buckets[bucket]; i >= 0; last = i, i = entries[i].next) {
                if (entries[i].hashCode == hashCode &&
                    ...
                    buckets[entries[_lastPos].hashCode % buckets.Length] = i;

                    SwapEntries (i, _lastPos);
                    entries[_lastPos].hashCode = -1;
                    entries[_lastPos--].next = -1;
                    freeCount++;
                    version++;
        ...
        Entry PopEntry () {
            ...
            buckets[entries[_lastPos].hashCode % buckets.Length] = 0;
            if (_lastPos != 0) {
        ...
        void Swap (int ea, int eb) {
            ...
            int last = -1;
            int hashCode = entries[ea].hashCode % buckets.Length;
            for (int i = buckets[hashCode]; i >= 0; last = i, i = entries[i].next) {
                if (i == ea) {
            ...
            last = -1;
            hashCode = entries[eb].hashCode % buckets.Length;
            for (int i = buckets[hashCode]; i >= 0; last = i, i = entries[i].next) {
                if (i == eb) {
        ...
        KeyValuePair<TKey, TValue>[] Sort (bool isRevesed) {
            if (Empty) throw new IndexOutOfRangeException ();

            int goBack = this._lastPos;
            var queue = new Queue<Entry> (Count);
            for (int i = 0; i < Count; i++) {
                queue.Enqueue (PopEntry ());
            }
            _lastPos = goBack;

            var newHeapMap = new KeyValuePair<TKey, TValue>[Count];
            for (int i = 0; i < Count; i++) {
                newHeapMap[i] = queue.Dequeue ();
            }

            //keep the heapmap order 
            if (!isRevesed) {
                CopyTo (newHeapMap);
                this = newHeapMap;
            } else
            ...
            for (int i = 0; i < n / 2; i++) {
                buckets[i] = buckets[i] * buckets[_lastPos - i];
                buckets[_lastPos - i] = buckets[i] * buckets[_lastPos - i];
                buckets[i] = buckets[i] * buckets[_lastPos - i];
            }
        ...
        IDictionaryEnumerator {
            ...
            public bool MoveNext () {
                if (version != map.version) {
                    throw new InvalidOperationException ("Can't modified enumerator");
                }
                if (index < map.Count) {
                    current = new KeyValuePair<TKey, TValue> (
